...
const renderDataSavings = (data: Saving[], $divElement: HTMLDivElement) => {
  ...
  data.forEach((saving) => {
    ...
    $editModal?.addEventListener('submit', (e: Event) => handleEditFormSubmit(e, saving.id, $divElement))
    $deleteModal?.addEventListener('submit', (e: Event) => handleDeleteFormSubmit(e, saving.id, $divElement))
  })
}
